            ...
            while True:
                try:
                    ...
                    for item in items:
                        ...
                        # 位置
                        location = item.xpath('td[5]/text()')[0].strip()
                        proxy = f'{ip}:{port}'
                        is_useful = await self.test_proxy(proxy)
                        if is_useful:
                            logging.info(f'数据源:快代理 成功获取一条ip')
                            now_time = int(time.time())
                            data = {'ip': proxy, 'anonymity': anonymity, 'ip_type': ip_type, 'location': location,
                                    'market': '快代理', 'get_time': now_time, 'last_check': now_time, 'live_time': 0}
